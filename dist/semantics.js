...
export function firstByte(addr) {
...
export const flipUlBit = (addr) => firstByte(addr) ^ 0b00000010;
export const setUlBit = (addr) => firstByte(addr) | 0b00000010;
export const clearUlBit = (addr) => firstByte(addr) & ~0b00000010;
export const clearIgBit = (addr) => firstByte(addr) & ~0b00000001;
/** EUI-48 -> EUI-64: insert ff:fe and flip U/L on first byte. */
export function toEui64FromEui48(addr) {
    const p = addr instanceof Uint8Array ? addr : addr.packed;
    if (p.length !== 6)
        throw new Error("toEui64FromEui48: input must be 48-bit");
...
export function toEui48FromEui64IfConvertible(addr) {
    const p = addr instanceof Uint8Array ? addr : addr.packed;
    if (p.length !== 8)
        return null;
    if (p[3] !== 0xff || p[4] !== 0xfe)
        return null;
